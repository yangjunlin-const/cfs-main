 ...
 * <p>
 * The HardLink class was formerly a static inner class of FSUtil, and the methods provided were blatantly
 * non-thread-safe. To enable volume-parallel Update snapshots, we now provide static threadsafe methods that allocate
 * new buffer string arrays upon each call.  We also provide an API to hardlink all files in a directory with a single
 * command, which is up to 128 times more efficient - and minimizes the impact of the extra buffer creations.
 */
public class HardLink {
    ...
    /**
     * This abstract class bridges the OS-dependent implementations of the needed functionality for querying link
     * counts. The particular implementation class is chosen during static initialization phase of the HardLink class.
   ...
   */
    ...
    /**
     * HardLink statistics counters and methods. Not multi-thread safe, obviously. Init is called during HardLink
     * instantiation, above.
     * <p>
     * These are intended for use by knowledgeable clients, not internally, because many of the internal methods are
     * static and can't update these per-instance counters.
     */
