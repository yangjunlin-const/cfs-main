...
import com.buaa.cfs.utils.IOUtils;
...
/**
 * The FileContext class provides an interface to the application writer for using the Hadoop file system. It provides a
 * set of methods for the usual operation: create, open, list, etc
 * <p>
 * <p>
 * <b> *** Path Names *** </b>
 * <p>
 * <p>
 * The Hadoop file system supports a URI name space and URI names. It offers a forest of file systems that can be
 * referenced using fully qualified URIs. Two common Hadoop file systems implementations are <ul> <li> the local file
 * system: file:///path <li> the hdfs file system hdfs://nnAddress:nnPort/path </ul>
 * <p>
 * While URI names are very flexible, it requires knowing the name or address of the server. For convenience one often
 * wants to access the default system in one's environment without knowing its name/address. This has an additional
 * benefit that it allows one to change one's default fs (e.g. admin moves application from cluster1 to cluster2).
 * <p>
 * <p>
 * To facilitate this, Hadoop supports a notion of a default file system. The user can set his default file system,
 * although this is typically set up for you in your environment via your default config. A default file system implies
 * a default scheme and authority; slash-relative names (such as /for/bar) are resolved relative to that default FS.
 * Similarly a user can also have working-directory-relative names (i.e. names not starting with a slash). While the
 * working directory is generally in the same default FS, the wd can be in a different FS.
 * <p>
 * Hence Hadoop path names can be one of: <ul> <li> fully qualified URI: scheme://authority/path <li> slash relative
 * names: /path relative to the default file system <li> wd-relative names: path  relative to the working dir </ul>
 * Relative paths with scheme (scheme:foo/bar) are illegal.
 * <p>
 * <p>
 * <b>****The Role of the FileContext and configuration defaults****</b>
 * <p>
 * The FileContext provides file namespace context for resolving file names; it also contains the umask for permissions,
 * In that sense it is like the per-process file-related state in Unix system. These two properties <ul> <li> default
 * file system i.e your slash) <li> umask </ul> in general, are obtained from the default configuration file in your
 * environment,  (@see {@link Configuration}).
 * <p>
 * No other configuration parameters are obtained from the default config as far as the file context layer is concerned.
 * All file system instances (i.e. deployments of file systems) have default properties; we call these server side (SS)
 * defaults. Operation like create allow one to select many properties: either pass them in as explicit parameters or
 * use the SS properties.
 * <p>
 * The file system related SS defaults are <ul> <li> the home directory (default is "/user/userName") <li> the initial
 * wd (only for local fs) <li> replication factor <li> block size <li> buffer size <li> encryptDataTransfer <li>
 * checksum option. (checksumType and  bytesPerChecksum) </ul>
 * <p>
 * <p>
 * <b> *** Usage Model for the FileContext class *** </b>
 * <p>
 * Example 1: use the default config read from the $HADOOP_CONFIG/core.xml. Unspecified values come from
 * core-defaults.xml in the release jar. <ul> <li> myFContext = FileContext.getFileContext(); // uses the default config
 * // which has your default FS <li>  myFContext.create(path, ...); <li>  myFContext.setWorkingDir(path) <li>
 * myFContext.open (path, ...); </ul> Example 2: Get a FileContext with a specific URI as the default FS <ul> <li>
 * myFContext = FileContext.getFileContext(URI) <li> myFContext.create(path, ...); ... </ul> Example 3: FileContext with
 * local file system as the default <ul> <li> myFContext = FileContext.getLocalFSFileContext() <li>
 * myFContext.create(path, ...); <li> ... </ul> Example 4: Use a specific config, ignoring $HADOOP_CONFIG Generally you
 * should not need use a config unless you are doing <ul> <li> configX = someConfigSomeOnePassedToYou. <li> myFContext =
 * getFileContext(configX); // configX is not changed, // is passed down <li> myFContext.create(path, ...); <li>...
 * </ul>
 */
...
public class FileContext {
    ...
    /**
     * Default permission for directory and symlink In previous versions, this default permission was also used to
     * create files, so files created end up with ugo+x permission. See HADOOP-9155 for detail. Two new constants are
     * added to solve this, please use {@link FileContext#DIR_DEFAULT_PERM} for directory, and use {@link
     * FileContext#FILE_DEFAULT_PERM} for file. This constant is kept for compatibility.
     */
    ...
    /**
     * The FileContext is defined by. 1) defaultFS (slash) 2) wd 3) umask
     */
     ...
     *
     * @throws UnsupportedFileSystemException If the file system for <code>absOrFqPath</code> is not supported.
     *                                        instantiated.
     */
     ...
     * @param defaultFS
     *
     * @return a FileContext with the specified AbstractFileSystem as the default FS.
     */
    ...
    /**
     * Create a FileContext using the default config read from the $HADOOP_CONFIG/core.xml, Unspecified key-values for
     * config are defaulted from core-defaults.xml in the release jar.
     *
     * @throws UnsupportedFileSystemException If the file system from the default configuration is not supported
     */
     ...
     *
     * @throws UnsupportedFileSystemException If the file system for {@link FsConstants#LOCAL_FS_URI} is not supported.
     */
     ...
     *
     * @throws UnsupportedFileSystemException If the file system for <code>defaultFsUri</code> is not supported
     */
     ...
     *
     * @throws UnsupportedFileSystemException If the file system with specified is not supported
     * @throws RuntimeException               If the file system specified is supported but could not be instantiated,
     *                                        or if login fails.
     */
    public static FileContext getFileContext(final URI defaultFsUri,
    ...
    /**
     * Create a FileContext using the passed config. Generally it is better to use {@link #getFileContext(URI,
     * Configuration)} instead of this one.
     *
     * @param aConf
     *
     * @return new FileContext
     *
     * @throws UnsupportedFileSystemException If file system in the config is not supported
     */
    public static FileContext getFileContext(final Configuration aConf)
            throws UnsupportedFileSystemException {
        final URI defaultFsUri = URI.create(aConf.get(CommonConfigurationKeysPublic.FS_DEFAULT_NAME_KEY,
        ...
        throw new UnsupportedFileSystemException(String.format(
                ...
                defaultFsUri, CommonConfigurationKeysPublic.FS_DEFAULT_NAME_KEY));
    }
     ...
     *
     * @throws UnsupportedFileSystemException If default file system in the config is not supported
     */
    ...
    /**
     * Set the working directory for wd-relative names (such a "foo/bar"). Working directory feature is provided by
     * simply prefixing relative names with the working dir. Note this is different from Unix where the wd is actually
     * set to the inode. Hence setWorkingDir does not follow symlinks etc. This works better in a distributed
     * environment that has multiple independent roots. {@link #getWorkingDirectory()} should return what
     * setWorkingDir() set.
     *
     * @param newWDir new working directory
     *
     * @throws IOException <br> NewWdir can be one of: <ul> <li>relative path: "foo/bar";</li> <li>absolute without
     *                     scheme: "/foo/bar"</li> <li>fully qualified with scheme: "xx://auth/foo/bar"</li> </ul> <br>
     *                     Illegal WDs: <ul> <li>relative with scheme: "xx:foo/bar"</li> <li>non existent directory</li>
     *                     </ul>
     */
    ...
    public FSDataOutputStream create(final Path f,
        ...
        final Options.CreateOpts[] updatedOpts =
                Options.CreateOpts.setOpt(Options.CreateOpts.perms(permission), opts);
