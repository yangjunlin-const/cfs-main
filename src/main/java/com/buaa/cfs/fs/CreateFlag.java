...
import com.buaa.cfs.exception.FileAlreadyExistsException;
import com.buaa.cfs.exception.HadoopIllegalArgumentException;
...
/****************************************************************
 * CreateFlag specifies the file create semantic. Users can combine flags like: <br> <code>
 * EnumSet.of(CreateFlag.CREATE, CreateFlag.APPEND) <code>
 * <p>
 * <p>
 * Use the CreateFlag as follows: <ol> <li> CREATE - to create a file if it does not exist, else throw
 * FileAlreadyExists.</li> <li> APPEND - to append to a file if it exists, else throw FileNotFoundException.</li> <li>
 * OVERWRITE - to truncate a file if it exists, else throw FileNotFoundException.</li> <li> CREATE|APPEND - to create a
 * file if it does not exist, else append to an existing file.</li> <li> CREATE|OVERWRITE - to create a file if it does
 * not exist, else overwrite an existing file.</li> <li> SYNC_BLOCK - to force closed blocks to the disk device. In
 * addition {@link Syncable#hsync()} should be called after each write, if true synchronous behavior is required.</li>
 * <li> LAZY_PERSIST - Create the block on transient storage (RAM) if available.</li> <li> APPEND_NEWBLOCK - Append data
 * to a new block instead of end of the last partial block.</li> </ol>
 * <p>
 * Following combination is not valid and will result in <ol> <li> APPEND|OVERWRITE</li> <li>
 * CREATE|APPEND|OVERWRITE</li> </ol>
 *****************************************************************/
public enum CreateFlag {
    ...
    /**
     * Create the block on transient storage (RAM) if available. If transient storage is unavailable then the block will
     * be created on disk.
     * <p>
     * HDFS will make a best effort to lazily write these files to persistent storage, however file contents may be lost
     * at any time due to process/ node restarts, hence there is no guarantee of data durability.
     * <p>
     * This flag must only be used for intermediate data whose loss can be tolerated by the application.
     */
