...
import com.buaa.cfs.exception.UnresolvedLinkException;

import java.io.IOException;
import java.net.URI;

/**
 * Used primarily by {@link FileContext} to operate on and resolve symlinks in a path. Operations can potentially span
 * multiple {@link AbstractFileSystem}s.
 *
 */
public abstract class FSLinkResolver<T> {

    /**
     * Return a fully-qualified version of the given symlink target if it has no scheme and authority. Partially and
     * fully-qualified paths are returned unmodified.
     *
     ...
     * using the given path which may result in an UnresolvedLinkException.
     *
     ...
     *
     * @throws UnresolvedLinkException If symbolic link <code>path</code> could not be resolved
    ...
    /**
     * Performs the operation specified by the next function, calling it repeatedly until all symlinks in the given path
     * are resolved.
     *
