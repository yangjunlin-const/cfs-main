...
/**
 * An AclStatus contains the ACL information of a specific file. AclStatus instances are immutable. Use a {@link
...
public class AclStatus {
    ...
    /**
     * Get the effective permission for the AclEntry. <br> Recommended to use this API ONLY if client communicates with
     * the old NameNode, needs to pass the Permission for the path to get effective permission, else use {@link
     * AclStatus#getEffectivePermission(AclEntry)}.
     *
     * @param entry   AclEntry to get the effective action
     * @param permArg Permission for the path. However if the client is NOT communicating with old namenode, then this
     *                argument will not have any preference.
     *
     * @return Returns the effective permission for the entry.
     *
     * @throws IllegalArgumentException If the client communicating with old namenode and permission is not passed as an
     *                                  argument.
     */
