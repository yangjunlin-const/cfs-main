...
public class ContentSummary implements Writable {
    ...
    /**
     * Constructor, deprecated by ContentSummary.Builder This constructor implicitly set spaceConsumed the same as
     * length. spaceConsumed and length must be set explicitly with ContentSummary.Builder
     */
    @Deprecated
    ...
    /**
     * Output format: <----12----> <----12----> <-------18-------> DIR_COUNT   FILE_COUNT       CONTENT_SIZE FILE_NAME
     */
    private static final String STRING_FORMAT = "%12s %12s %18s ";
    /**
     * Output format: <----12----> <----15----> <----15----> <----15----> <----12----> <----12----> <-------18------->
    ...
    /**
     * Return the header of the output. if qOption is false, output directory count, file count, and content size; if
     * qOption is true, output quota and remaining quota as well.
     *
    ...
    /**
     * Return the string representation of the object in the output format. if qOption is false, output directory count,
     * file count, and content size; if qOption is true, output quota and remaining quota as well.
     *
     * @param qOption a flag indicating if quota needs to be printed or not
     *
    ...
    /**
     * Return the string representation of the object in the output format. if qOption is false, output directory count,
     * file count, and content size; if qOption is true, output quota and remaining quota as well. if hOption is false
     * file sizes are returned in bytes if hOption is true file sizes are returned in human readable
     *
     * @param qOption a flag indicating if quota needs to be printed or not
