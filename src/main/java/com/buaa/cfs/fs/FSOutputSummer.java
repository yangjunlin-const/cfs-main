...
abstract public class FSOutputSummer extends OutputStream {
    ...
    /**
     * Check if the implementing OutputStream is closed and should no longer accept writes. Implementations should do
     * nothing if this stream is not closed, and should throw an {@link IOException} if it is closed.
     *
    ...
    /**
     * Writes <code>len</code> bytes from the specified byte array starting at offset <code>off</code> and generate a
     * checksum for each data chunk.
     * <p>
     * <p> This method stores bytes from the given array into this stream's buffer before it gets checksumed. The buffer
     * gets checksumed and flushed to the underlying output stream when all data in a checksum chunk are in the buffer.
     * If the buffer is empty and requested length is at least as large as the size of next checksum chunk size, this
     * method will checksum and write the chunk directly to the underlying output stream.  Thus it avoids uneccessary
     * data copy.
     *
     * @param b   the data.
    ...
    /**
     * Checksums all complete data chunks and flushes them to the underlying stream. If there is a trailing partial
     * chunk, it is not flushed and is maintained in the buffer.
     */
