...
/**
 * User and group information for Hadoop. This class wraps around a JAAS Subject and provides methods to determine the
 * user's username and groups. It supports both the Windows, Unix and Kerberos login modules.
 */
public class UserGroupInformation {
    ...
    /**
     * For the purposes of unit tests, we want to test login from keytab and don't want to wait until the renew window
     * (controlled by TICKET_RENEW_WINDOW).
     *
...
//
//    /**
//     * UgiMetrics maintains UGI activity statistics and publishes them through the metrics interfaces.
//     */
...
//    static UgiMetrics metrics = UgiMetrics.create();
    ...
    /**
     * A method to initialize the fields that depend on a configuration. Must be called before useKerberos or groups is
     * used.
     */
...
//        }
    ...
    /**
     * Set the static configuration for UGI. In particular, set the security authentication mechanism and the group look
     * up service.
     *
     * @param conf the configuration to use
     */
    ...
    /**
     * Determine if UserGroupInformation is using Kerberos to determine user identities or is relying on simple
     * authentication
     *
     ...
     *
     * @param ticketCachePath The Kerberos ticket cache path, or NULL if none is specfied
     ...
     *
     * @throws IOException if login fails
     * @parma subject the subject to use when logging in a user, or null to create a new subject.
     */
...
//                loginUser.addCredentials(cred);
    ...
    /**
     * Log a user in from a keytab file. Loads a user identity from a keytab file and logs them in. They become the
...
//                metrics.loginFailure.add(Time.now() - start);
    ...
    /**
     * Re-Login a user in from a keytab file. Loads a user identity from a keytab file and logs them in. They become the
     * currently logged-in user. This method assumes that {@link #loginUserFromKeytab(String, String)} had happened
     * already. The Subject field of this UserGroupInformation object is updated to have the new credentials.
     *
...
//                metrics.loginFailure.add(Time.now() - start);
    ...
    /**
     * Re-Login a user in from the ticket cache.  This method assumes that login had happened already. The Subject field
     * of this UserGroupInformation object is updated to have the new credentials.
     *
     * @throws IOException on a failure
     */
    ...
    /**
     * Log a user in from a keytab file. Loads a user identity from a keytab file and login them in. This new user does
     * not affect the currently logged-in user.
     *
     * @param user the principal name to load from the keytab
     * @param path the path to the keytab file
     *
...
//                metrics.loginFailure.add(Time.now() - start);
    ...
    /**
     * Create a user from a login name. It is intended to be used for remote users in RPC, since it won't have any
     * credentials.
     *
...
//    }
    ...
    /**
     * This class is used for storing the groups for testing. It stores a local map that has the translation of
     * usernames to groups.
     */
     ...
     *
     * @param user       the full user principal name for effective user
     * @param realUser   UGI of the real user
     * @param userGroups the names of the groups that the user belongs to
     *
     * @return a fake user for running unit tests
     */
    ...
    /**
     * Add a TokenIdentifier to this UGI. The TokenIdentifier has typically been authenticated by the RPC layer as
     * belonging to the user represented by this UGI.
     *
...
//        return credentials;
     ...
     *
     * @return the list of users with the primary group first. If the command fails, it returns an empty list.
     */
...
//    }
    ...
    /**
     * Get the authentication method from the real user's subject.  If there is no real user, return the given user's
    ...
    /**
     * Returns the authentication method of a ugi. If the authentication method is PROXY, returns the authentication
     * method of the real user.
     *
     * @param ugi
     *
