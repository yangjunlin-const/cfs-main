...
import com.buaa.cfs.exception.RetriableException;
import com.buaa.cfs.exception.StandbyException;
...
public abstract class SecretManager<T extends TokenIdentifier> {
    ...
    /**
     * Retrieve the password for the given token identifier. Should check the date or registry to make sure the token
     * hasn't expired or been revoked. Returns the relevant password.
     *
    ...
    /**
     * The same functionality with {@link #retrievePassword}, except that this method can throw a {@link
     * RetriableException} or a {@link StandbyException} to indicate that client can retry/failover the same operation
     * because of temporary issue on the server side.
     *
     * @param identifier the identifier to validate
     *
     * @return the password to use
     *
     * @throws InvalidToken       the token was invalid
     * @throws StandbyException   the server is in standby state, the client can try other servers
     * @throws RetriableException the token was invalid, and the server thinks this may be a temporary issue and
     *                            suggests the client to retry
     * @throws IOException        to allow future exceptions to be added without breaking compatibility
     */
     ...
     *
     * @throws StandbyException if the secret manager is not available to read tokens
     */
