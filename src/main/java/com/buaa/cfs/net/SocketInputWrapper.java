...
/**
 * A wrapper stream around a socket which allows setting of its timeout. If the socket has a channel, this uses
 * non-blocking IO via the package-private {@link SocketInputStream} implementation. Otherwise, timeouts are managed by
...
public class SocketInputWrapper extends FilterInputStream {
     ...
     * <p>
     * Note: the behavior here can differ subtly depending on whether the underlying socket has an associated Channel.
     * In particular, if there is no channel, then this call will affect the socket timeout for <em>all</em> readers of
     * this socket. If there is a channel, then this call will affect the timeout only for <em>this</em> stream. As
     * such, it is recommended to only create one {@link SocketInputWrapper} instance per socket.
     *
     * @param timeoutMs the new timeout, 0 for no timeout
     *
