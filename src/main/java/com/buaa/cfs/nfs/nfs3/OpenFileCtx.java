...
import com.buaa.cfs.fs.FSDataInputStream;
import com.buaa.cfs.nfs.request.WRITE3Request;
import com.buaa.cfs.nfs.response.WRITE3Response;
import com.buaa.cfs.nfs.response.WccAttr;
import com.buaa.cfs.nfs.response.WccData;
import com.buaa.cfs.security.IdMappingServiceProvider;
import com.buaa.cfs.utils.Daemon;
...
/**
 * OpenFileCtx saves the context of one HDFS file output stream. Access to it is synchronized by its member lock.
 */
class OpenFileCtx {
    ...
    public enum COMMIT_STATUS {
        ...
        COMMIT_DO_SYNC,
        /**
         * Deferred COMMIT response could fail file uploading. The following two status are introduced as a solution. 1.
         * if client asks to commit non-sequential trunk of data, NFS gateway return success with the hope that client
         * will send the prerequisite writes. 2. if client asks to commit a sequential trunk(means it can be flushed to
         * HDFS), NFS gateway return a special error NFS3ERR_JUKEBOX indicating the client needs to retry. Meanwhile,
         * NFS gateway keeps flush data to HDFS and do sync eventually.
         * <p>
         * The reason to let client wait is that, we want the client to wait for the last commit. Otherwise, client
         * thinks file upload finished (e.g., cp command returns success) but NFS could be still flushing staged data to
         * HDFS. However, we don't know which one is the last commit. We make the assumption that a commit after
         * sequential writes may be the last. Referring HDFS-7259 for more details.
         */
    ...
    /**
     * The current offset of the file in HDFS. All the content before this offset has been written back to HDFS.
     */
    private AtomicLong nextOffset;
    //  private final HdfsDataOutputStream fos;
    private final DataOutputStream fos;
    ...
    OpenFileCtx(DataOutputStream fos, Nfs3FileAttributes latestAttr,
...
//        return fos.getPos();
    ...
    /**
     * Creates and adds a WriteCtx into the pendingWrites map. This is a synchronized method to handle concurrent
     * writes.
     *
     * @return A non-null {@link WriteCtx} instance if the incoming write request's offset >= nextOffset. Otherwise
     * null.
     */
    private synchronized WriteCtx addWritesToCache(WRITE3Request request,
        ...
        if (offset < cachedOffset) {
            ...
            WriteCtx.DataState dataState = offset == cachedOffset ? WriteCtx.DataState.NO_DUMP
    ...
    private void processOverWrite(DFSClient dfsClient, WRITE3Request request,
        ...
        if (offset + count > cachedOffset) {
            ...
            response = new WRITE3Response(Nfs3Status.NFS3ERR_INVAL, wccData, 0,
                    Nfs3Constant.WriteStableHow.UNSTABLE, Nfs3Constant.WRITE_COMMIT_VERF);
        } else {
            if (LOG.isDebugEnabled()) {
    ...
    /**
     * Check if we can start the write (back to HDFS) now. If there is no hole for writing, and there is no other
     * threads writing (i.e., asyncStatus is false), start the writing and set asyncStatus to true.
     *
     * @return True if the new write is sequential and we can start writing (including the case that there is already a
     * thread writing).
     */
...
//                        .getElapsedTime(writeCtx.startTime));
    ...
    /**
     * Honor 2 kinds of overwrites: 1). support some application like touch(write the same content back to change
     * mtime), 2) client somehow sends the same write again in a different RPC.
     */
    private WRITE3Response processPerfectOverWrite(DFSClient dfsClient,
            long offset, int count, Nfs3Constant.WriteStableHow stableHow, byte[] data,
...
//        }
     ...
     *
     * @return one commit status: COMMIT_FINISHED, COMMIT_WAIT, COMMIT_INACTIVE_CTX, COMMIT_INACTIVE_WITH_PENDING_WRITE,
     * COMMIT_ERROR
     */
...
//            }
     ...
     *
     * @return Null if {@link #pendingWrites} is null, or the next WriteCtx's offset is larger than nextOffSet.
     */
...
//                            fos.hsync();
//                        } else {
//                            Preconditions.checkState(stableHow == Nfs3Constant.WriteStableHow.FILE_SYNC,
