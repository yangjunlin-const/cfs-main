...
import com.buaa.cfs.nfs.common.oncrpc.*;
...
final class RpcProgramPortmapTCP extends SimpleChannelInboundHandler {
    ...
    private static final Log LOG = LogFactory.getLog(RpcProgramPortmapTCP.class);

    /** ChannelGroup that remembers all active channels for gracefully shutdown. */

    RpcProgramPortmapTCP() {
        ...
        RpcProgramPortmapUDP.map.put(PortmapMapping.key(m), m);
        RpcProgramPortmapUDP.map.put(PortmapMapping.key(m1), m1);
    }

    /**
     * This procedure does no work. By convention, procedure zero of any protocol takes no parameters and returns no
     * results.
     */
    ...
    /**
     * When a program first becomes available on a machine, it registers itself with the port mapper program on the same
     * machine. The program passes its program number "prog", version number "vers", transport protocol number "prot",
     * and the port "port" on which it awaits service request. The procedure returns a boolean reply whose value is
     * "TRUE" if the procedure successfully established the mapping and "FALSE" otherwise. The procedure refuses to
     * establish a mapping if one already exists for the tuple "(prog, vers, prot)".
     */
    private XDR set(int xid, XDR in, XDR out) {
        ...
        RpcProgramPortmapUDP.map.put(key, mapping);
     ...
     * The parameters and results have meanings identical to those of "PMAPPROC_SET". The protocol and port number
    ...
    private XDR unset(int xid, XDR in, XDR out) {
        ...
        RpcProgramPortmapUDP.map.remove(key);
        return PortmapResponse.booleanReply(out, xid, true);
    }

    /**
     * Given a program number "prog", version number "vers", and transport protocol number "prot", this procedure
     * returns the port number on which the program is awaiting call requests. A port value of zeros means the program
    ...
    private XDR getport(int xid, XDR in, XDR out) {
        ...
        PortmapMapping value = RpcProgramPortmapUDP.map.get(key);
        int res = 0;
        if (value != null) {
            res = value.getPort();
            if (LOG.isDebugEnabled()) {
    ...
    /**
     * This procedure enumerates all entries in the port mapper's database. The procedure takes no parameters and
     * returns a list of program, version, protocol, and port values.
     */
    private XDR dump(int xid, XDR in, XDR out) {
        PortmapMapping[] pmapList = RpcProgramPortmapUDP.map.values().toArray(new PortmapMapping[0]);
