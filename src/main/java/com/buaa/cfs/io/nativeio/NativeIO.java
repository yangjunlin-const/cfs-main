...
/**
 * JNI wrappers for various native IO-related calls not available in Java. These functions should generally be used
 * alongside a fallback to another more portable mechanism.
 */
public class NativeIO {
    ...
    /* No further special treatment.  */
        ...
        /**
         * A CacheManipulator used for testing which does not actually call mlock. This allows many tests to be run even
         * when the operating system does not allow mlock, or only allows limited mlocking.
         */
        @VisibleForTesting
        ...
        /**
         * Call posix_fadvise on the given file descriptor. See the manpage for this syscall for more information. On
         * systems where this call is not available, does nothing.
         *
        ...
        /**
         * Call sync_file_range on the given file descriptor. See the manpage for this syscall for more information. On
         * systems where this call is not available, does nothing.
         *
         * @throws NativeIOException if there is an error with the syscall
         */
        ...
        /**
         * Locks the provided direct ByteBuffer into memory, preventing it from swapping out. After a buffer is locked,
         * future accesses will not incur a page fault.
         * <p>
         ...
         * <p>
         * There isn't any portable way to unmap a memory region in Java. So we use the sun.nio method here. Note that
         * unmapping a memory region could cause crashes if code continues to reference the unmapped code.  However, if
         * we don't manually unmap the memory, we are dependent on the finalizer to do it, and we have no idea when the
         * finalizer will run.
         *
    ...
    public static class Windows {
        ...
        /**
         * Create a directory with permissions set to the specified mode.  By setting permissions at creation time, we
         * avoid issues related to the user lacking WRITE_DAC rights on subsequent chmod calls.  One example where this
         * can occur is writing to an SMB share where the user does not have Full Control rights, and therefore
         * WRITE_DAC is denied.
         *
        ...
        /**
         * Create a file for write with permissions set to the specified mode.  By setting permissions at creation time,
         * we avoid issues related to the user lacking WRITE_DAC rights on subsequent chmod calls.  One example where
         * this can occur is writing to an SMB share where the user does not have Full Control rights, and therefore
         * WRITE_DAC is denied.
         * <p>
         * This method mimics the semantics implemented by the JDK in {@link FileOutputStream}.  The file is opened for
         * truncate or append, the sharing mode allows other readers and writers, and paths longer than MAX_PATH are
         * supported.  (See io_util_md.c in the JDK.)
         *
         ...
         * <p>
         * Longer term this native function can be substituted with JDK7 function Files#isReadable, isWritable,
         * isExecutable.
         *
        ...
        /**
         * Extends both the minimum and maximum working set size of the current process.  This method gets the current
         * minimum and maximum working set size, adds the requested amount to each and then sets the minimum and maximum
         * working set size to the new values.  Controlling the working set size of the process also controls the amount
         * of memory it can lock.
         *
    ...
    /**
     * Get the maximum number of bytes that can be locked into memory at any given point.
     *
     * @return 0 if no bytes can be locked into memory; Long.MAX_VALUE if there is no limit; The number of bytes that
     * can be locked into memory otherwise.
     */
    ...
    /**
     * The Windows logon name has two part, NetBIOS domain name and user account name, of the format DOMAIN\UserName.
     * This method will remove the domain part of the full logon name.
     *
    ...
    /**
     * Create a FileInputStream that shares delete permission on the file opened, i.e. other process can delete the file
     * the FileInputStream is reading. Only Windows implementation uses the native interface.
     */
    ...
    /**
     * Create a FileInputStream that shares delete permission on the file opened at a given offset, i.e. other process
     * can delete the file the FileInputStream is reading. Only Windows implementation uses the native interface.
     */
