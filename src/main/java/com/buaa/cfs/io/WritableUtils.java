...
public final class WritableUtils {
    ...
    /**
     * Serializes an integer to a binary stream with zero-compressed encoding. For -120 <= i <= 127, only one byte is
     * used with the actual value. For other values of i, the first byte value indicates whether the integer is positive
     * or negative, and the number of bytes that follow. If the first byte value v is between -121 and -124, the
     * following integer is positive, with number of bytes that follow are -(v+120). If the first byte value v is
     * between -125 and -128, the following integer is negative, with number of bytes that follow are -(v+124). Bytes
     * are stored in the high-non-zero-byte-first order.
     *
    ...
    /**
     * Serializes a long to a binary stream with zero-compressed encoding. For -112 <= i <= 127, only one byte is used
     * with the actual value. For other values of i, the first byte value indicates whether the long is positive or
     * negative, and the number of bytes that follow. If the first byte value v is between -113 and -120, the following
     * long is positive, with number of bytes that follow are -(v+112). If the first byte value v is between -121 and
     * -128, the following long is negative, with number of bytes that follow are -(v+120). Bytes are stored in the
     ...
     * <p>
     * This function validates that the integer is between [lower, upper], inclusive.
     *
     * @param stream Binary input stream
     *
     * @return deserialized integer from stream
     ...
     * @throws IllegalArgumentException if the encoded byte size for string is negative or larger than maxSize. Only the
     *                                  vint is read.
     */
