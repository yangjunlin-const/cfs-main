...
/**
 * Wrapper around JNI support code to do checksum computation natively.
 */
class NativeCrc32 {
    ...
    /**
     * Verify the given buffers of data and checksums, and throw an exception if any checksum is invalid. The buffers
     * given to this function should have their position initially at the start of the data, and their limit set at the
     * end of the data. The position, limit, and mark are not modified.
     *
     * @param bytesPerSum  the chunk size (eg 512 bytes)
     * @param checksumType the DataChecksum type constant (NULL is not supported)
     * @param sums         the DirectByteBuffer pointing at the beginning of the stored checksums
     * @param data         the DirectByteBuffer pointing at the beginning of the data to check
     ...
     * Verify the given buffers of data and checksums, and throw an exception if any checksum is invalid. The buffers
     * given to this function should have their position initially at the start of the data, and their limit set at the
     * end of the data. The position, limit, and mark are not modified.  This method is retained only for
     * backwards-compatibility with prior jar versions that need the corresponding JNI function.
     *
     * @param bytesPerSum  the chunk size (eg 512 bytes)
     ...
     *
     * @throws ChecksumException if there is an invalid checksum
     * @deprecated use {@link #nativeComputeChunkedSums(int, int, ByteBuffer, int, ByteBuffer, int, int, String, long,
     * boolean)} instead
     */
    @Deprecated
    @VisibleForTesting
