...
/**
 * Class which sets up a simple thread which runs in a loop sleeping for a short interval of time. If the sleep takes
 * significantly longer than its target time, it implies that the JVM or host machine has paused processing, which may
 * cause other problems. If such a pause is detected, the thread logs a message.
 */
