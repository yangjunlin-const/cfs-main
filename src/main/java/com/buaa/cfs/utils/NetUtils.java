...
import com.buaa.cfs.net.SocketInputWrapper;
...
public class NetUtils {
     ...
     * @param conf the configuration
     *
     * @return the default socket factory as specified in the configuration or the JVM default socket factory if the
     * configuration does not contain a default socket factory property.
     */
    ...
    /**
     * Get the socket factory corresponding to the given proxy URI. If the given proxy URI corresponds to an absence of
     * configuration parameter, returns null. If the URI is malformed raises an exception.
     *
     * @param propValue the property which is the class name of the SocketFactory to instantiate; assumed non null and
     *                  non empty.
     *
    ...
    /**
     * Adds a static resolution for host. This can be used for setting up hostnames with names that are fake to point to
     * a well known host. For e.g. in some testcases we require to have daemons with different hostnames running on the
     * same machine. In order to create connections to these daemons, one can set up mappings from those hostnames to
     * "localhost". {@link NetUtils#getStaticResolution(String)} can be used to query for the actual hostname.
     *
     * @param host
     * @param resolvedName
     */
    ...
    /**
     * Retrieves the resolved name for the passed host. The resolved name must have been set earlier using {@link
...
//  }
    ...
    /**
     * Same as getOutputStream(socket, 0). Timeout of zero implies write will wait until data is available.<br><br>
     * <p>
     * From documentation for {@link #getOutputStream(Socket, long)} : <br> Returns OutputStream for the socket. If the
     * socket has an associated SocketChannel then it returns a {@link SocketOutputStream} with the given timeout. If
     * the socket does not have a channel, {@link Socket#getOutputStream()} is returned. In the later case, the timeout
     * argument is ignored and the write will wait until data is available.<br><br>
     * <p>
     * Any socket created using socket factories returned by {@link NetUtils}, must use this interface instead of {@link
    ...
    /**
     * Returns OutputStream for the socket. If the socket has an associated SocketChannel then it returns a {@link
     * SocketOutputStream} with the given timeout. If the socket does not have a channel, {@link
     * Socket#getOutputStream()} is returned. In the later case, the timeout argument is ignored and the write will wait
     * until data is available.<br><br>
     * <p>
     * Any socket created using socket factories returned by {@link NetUtils}, must use this interface instead of {@link
     * Socket#getOutputStream()}.
     *
     * @param socket
     * @param timeout timeout in milliseconds. This may not always apply. zero for waiting as long as necessary.
     *
     * @return OutputStream for writing to the socket.
     *
     * @throws IOException
...
//                socket.getOutputStream() : new SocketOutputStream(socket, timeout);
    ...
    /**
     * This is a drop-in replacement for {@link Socket#connect(SocketAddress, int)}. In the case of normal sockets that
     * don't have associated channels, this just invokes <code>socket.connect(endpoint, timeout)</code>. If
     * <code>socket.getChannel()</code> returns a non-null channel, connect is implemented using Hadoop's selectors.
     * This is done mainly to avoid Sun's connect implementation from creating thread-local selectors, since Hadoop does
     * not have control on when these are closed and could end up taking all the available file descriptors.
     *
     * @param socket
...
//      throw new ConnectTimeoutException(ste.getMessage());
     ...
     *
     * @param name a string representation of a host: either a textual representation its IP address or its host name
     *
    ...
    /**
     * Given a collection of string representation of hosts, return a list of corresponding IP addresses in the textual
     * representation.
     *
    ...
    /**
     * Take an IOException , the local host port and remote host port details and return an IOException with the input
     * exception as the cause and also include the host details. The new exception provides the stack trace of the place
     * where the exception is thrown and some extra diagnostics information. If the exception is BindException or
     * ConnectException or UnknownHostException or SocketTimeoutException, return a new one of the same type; Otherwise
