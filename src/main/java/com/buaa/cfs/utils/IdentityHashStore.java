...
/**
 * The IdentityHashStore stores (key, value) mappings in an array. It is similar to java.util.HashTable, but much more
 * lightweight. Neither inserting nor removing an element ever leads to any garbage getting created (assuming the array
 * doesn't need to be enlarged).
 * <p>
 * Unlike HashTable, it compares keys using {@link System#identityHashCode(Object)} and the identity operator. This is
 * useful for types like ByteBuffer which have expensive hashCode and equals operators.
 * <p>
 * We use linear probing to resolve collisions.  This avoids the need for the overhead of linked list data structures.
 * It also means that it is expensive to attempt to remove an element that isn't there, since we have to look at the
 * entire array to be sure that it doesn't exist.
 *
...
public final class IdentityHashStore<K, V> {
    /**
     * Even elements are keys; odd elements are values. The array has size 1 + Math.pow(2, capacity).
     */
     ...
     * <p>
     * Inserting a new (key, value) never overwrites a previous one. In other words, you can insert the same key
     * multiple times and it will lead to multiple entries.
     */
