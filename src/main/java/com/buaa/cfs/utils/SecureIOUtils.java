...
/**
 * This class provides secure APIs for opening and creating files on the local disk. The main issue this class tries to
 * handle is that of symlink traversal. <br/> An example of such an attack is: <ol> <li> Malicious user removes his
 * task's syslog file, and puts a link to the jobToken file of a target user.</li> <li> Malicious user tries to open the
 * syslog file via the servlet on the tasktracker.</li> <li> The tasktracker is unaware of the symlink, and simply
 * streams the contents of the jobToken file. The malicious user can now access potentially sensitive map outputs, etc.
 * of the target user's job.</li> </ol> A similar attack is possible involving task log truncation, but in that case due
 * to an insecure write to a file. <br/>
 */
public class SecureIOUtils {
    ...
    /**
     * Open the given File for random read access, verifying the expected user/ group constraints if security is
     * enabled.
     * <p>
     * Note that this function provides no additional security checks if hadoop security is disabled, since doing the
     * checks would be too expensive when native libraries are not available.
     *
     ...
     *
     * @throws IOException if an IO error occurred or if the user/group does not match when security is enabled.
     */
    ...
    /**
     * Opens the {@link FSDataInputStream} on the requested file on local file system, verifying the expected user/group
     ...
     * @param expectedGroup the expected group owner for the file
     *
     * @throws IOException if an IO Error occurred or the user/group does not match if security is enabled
     */
     ...
     * <p>
     * Note that this function provides no additional checks if Hadoop security is disabled, since doing the checks
     * would be too expensive when native libraries are not available.
     *
     * @param f             the file that we are trying to open
     * @param expectedOwner the expected user owner for the file
     * @param expectedGroup the expected group owner for the file
     *
     * @throws IOException if an IO Error occurred, or security is enabled and the user/group does not match
     */
    ...
    /**
     * Signals that an attempt to create a file at a given pathname has failed because another file already existed at
     * that path.
     */
