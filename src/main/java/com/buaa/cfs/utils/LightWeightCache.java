...
/**
 * A low memory footprint Cache which extends {@link LightWeightGSet}. An entry in the cache is expired if (1) it is
 * added to the cache longer than the creation-expiration period, and (2) it is not accessed for the access-expiration
 * period. When an entry is expired, it may be evicted from the cache. When the size limit of the cache is set, the
 * cache will evict the entries with earliest expiration time, even if they are not expired.
 * <p>
 * It is guaranteed that number of entries in the cache is less than or equal to the size limit.  However, It is not
 * guaranteed that expired entries are evicted from the cache. An expired entry may possibly be accessed after its
 ...
 * @param <E> Entry type, which must be (1) a subclass of K, and (2) implementing {@link Entry} interface, and
 */
public class LightWeightCache<K, E extends K> extends LightWeightGSet<K, E> {
     ...
     * @param sizeLimit                the limit of the size of the cache. The limit is disabled if it is <= 0.
     * @param creationExpirationPeriod the time period C > 0 in nanoseconds that the creation of an entry is expired if
     *                                 it is added to the cache longer than C.
     * @param accessExpirationPeriod   the time period A >= 0 in nanoseconds that the access of an entry is expired if
     *                                 it is not accessed longer than A.
     */
